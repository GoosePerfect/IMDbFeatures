...
When('I click on menu button', async () => {
    let clickOnMenuButton = await($('*[class="ipc-icon ipc-icon--menu ipc-button__icon ipc-button__icon--pre"]'));
    clickOnMenuButton.click();
});

When('I click on Most Popular Movie', async () => {
    let clickOnMostPopularMovie = await($('[href="/chart/moviemeter/?ref_=nv_mv_mpm"]'));
...
When('I click on the sort by dropdown menu', async () => {
    let clickOnDropDownMenu = await ($('*[class=lister-sort-by]'));
    clickOnDropDownMenu.click();
});

When('I select IMDbs rating', async () => {
    let clickOnIMDbsRating = await ($('*[value="ir:descending"]'));
...
When('I select Release date', async () => {
    let clickOnIMDbsRating = await ($('*[value="us:descending"]'));
    clickOnIMDbsRating.click();
});
...
When('I click on descending order', async () => {
    let clickOnDescendingOrder = await ($('*[data-sort="rk:desc"]'));
